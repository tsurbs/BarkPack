...
function SkillsPage() {
    ...
    const handleSelectSkill = async (id: string) => {
        try {
            ...
            const res = await fetch(`${BACKEND_URL}/v1/agents/${id}`);
                ...
                <div className="scotty-card overflow-hidden flex flex-col items-stretch">
                    ...
                    <div className="flex-1 overflow-y-auto p-2 space-y-1 bg-white">
                        {loading && skills.length === 0 ? (
                            <p className="text-black-400 p-4 text-center">Loading skills...</p>
                        ) : skills.length === 0 ? (
                            ...
                            skills.map(skill => (
                                ...
                                >
                                    <div className="flex justify-between items-center">
                                        ...
                                        <span className="text-xs bg-white border border-black-200 px-1.5 py-0.5 rounded text-black-400">v{skill.version}</span>
                                    </div>
                                    <p className="text-xs text-black-400 mt-1 truncate">{skill.description}</p>
                                </button>
                            ))
                        )}
                    </div>
                ...
                <div className="md:col-span-2 scotty-card p-0 flex flex-col overflow-hidden">
                    <div className="p-4 border-b border-black-100 bg-black-50 flex items-center justify-between">
                        <h3 className="font-bold text-black-900">
                            {selectedSkillId ? `Editing: ${selectedSkillId}` : "Create New Skill"}
                        </h3>
