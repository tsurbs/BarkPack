                                ...
                                >
                                    <div className="flex justify-between items-center">
                                        <span className="font-medium flex items-center gap-1.5">
                                            {tool.toolType === 'python' && <Code size={14} className="text-green-600" />}
                                            {tool.toolType === 'mcp' && <Server size={14} className="text-purple-600" />}
                                            {tool.toolType === 'native' && <Wrench size={14} className="text-black-400" />}
                                            {tool.name}
                                        </span>
                                    </div>
                                    <p className="text-xs text-black-400 mt-1 truncate">{tool.description}</p>
                                </button>
                            ))
                        )}
                    </div>
                    ...
                    <div className="scotty-card p-6 border-b border-black-100 bg-white">
                        <div className="flex justify-between items-start mb-4">
                            <h3 className="font-bold text-lg text-black-900">
                                {selectedToolId ? (isNative ? "View Native Tool" : "Edit Tool") : "Create New Tool"}
                            </h3>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-bold text-black-900 mb-1">Tool Name</label>
                                <input
                                    type="text"
                                    value={name}
                            ...
                            <div>
                                <label className="block text-sm font-bold text-black-900 mb-1">Type</label>
                                <select
                                    value={isNative ? "native" : toolType}
                                    onChange={(e) => {
                                        setToolType(e.target.value as any);
                                        if (e.target.value === "mcp") setContent('{\n  "command": "npx",\n  "args": ["-y", "@modelcontextprotocol/server-postgres", "postgresql://user:pass@localhost/db"],\n  "env": {}\n}');
                                        else if (e.target.value === "python") setContent(DEFAULT_PYTHON_CODE);
                                    }}
                                    disabled={isNative}
                                    className="w-full border border-black-200 rounded-md px-3 py-2 text-sm focus:outline-none focus:border-blue-500 disabled:bg-black-50 disabled:text-black-400"
                                >
                            ...
                            <div className="col-span-2">
                                <label className="block text-sm font-bold text-black-900 mb-1">Description</label>
                                <input
                                    type="text"
                                    value={description}
                    ...
                    <div className="flex-1 scotty-card p-0 flex flex-col overflow-hidden relative">
                        <div className="p-3 border-b border-black-100 bg-black-50 flex items-center justify-between">
                            <span className="font-bold text-sm text-black-900">
                                {isNative ? "Native Implementation Info" : toolType === 'python' ? 'Python Source Code' : 'MCP Configuration (JSON)'}
                            </span>
                        </div>
                        {isNative ? (
                            <div className="p-6 text-black-400 text-sm h-full flex items-center justify-center bg-black-50 text-center">
                                This tool is native to the application codebase.<br />
                            ...
                            <textarea
                                ...
                                spellCheck={false}
                                placeholder={toolType === 'python' ? "Write python here..." : "Enter JSON configuration..."}
                            />
                        )}
                    </div>
                </div>
